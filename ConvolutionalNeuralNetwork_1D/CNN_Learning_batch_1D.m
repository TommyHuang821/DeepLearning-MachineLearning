function CNN_net=CNN_Learning_batch_1D(traindata,train_out,CNN_net)
% traindata: image format (n_train * mapsize ), first two dimesnion represent a image, third dimension represents the number of training data.
...
for iter=1:numepochs  
   ...
   for l = 1 : numbatches
       ...
       CNN_net=CNN_feedforward_1D(batch_x,CNN_net);
       CNN_net=CNN_feedbackward_1D(batch_out,CNN_net);
       ...
       if isempty(CNN_net.rL)
        ...
        tmp=0;
        for i=1:batchsize
            errorvalue=errorvalue-log10( CNN_net.Output(i,(batch_out(i,:)==1)));
%             tmp=tmp-log10( CNN_net.Output(i,(batch_out(i,:)==1)));
        end
