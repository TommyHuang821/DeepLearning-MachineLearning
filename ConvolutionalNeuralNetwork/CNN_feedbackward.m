function [CNN_net]=CNN_feedbackward(batch_out,CNN_net)
...
%     CNN_net.LayerDesign{Num_Layer}.od=CNN_net.LayerDesign{Num_Layer}.e .*daf( CNN_net.Output_noaf); % delta: output
    ...
    for j = 1 : numel(CNN_net.LayerDesign{Num_Layer-1}.a)
        CNN_net.LayerDesign{Num_Layer-1}.d{j} = reshape(CNN_net.LayerDesign{Num_Layer}.fvd(((j - 1) * fvnum + 1) : j * fvnum, :), sa(1), sa(2), sa(3));
    end
     ...
     for iL = (Num_Layer - 2) : -1 : 1
        if strcmp(CNN_net.LayerDesign{iL}.LayerType, 'C')
            ...
            for j = 1 : numel(CNN_net.LayerDesign{iL}.a)
                ...
                tmp2=(ExpandMatrix(CNN_net.LayerDesign{iL + 1}.d{j}, [CNN_net.LayerDesign{iL + 1}.scale CNN_net.LayerDesign{iL + 1}.scale 1]) / CNN_net.LayerDesign{iL + 1}.scale ^ 2);
        ...
        if strcmp(CNN_net.LayerDesign{iL}.LayerType, 'C')
            for j = 1 : numel(CNN_net.LayerDesign{iL}.a)
                for i = 1 : numel(CNN_net.LayerDesign{iL - 1}.a)
                    CNN_net.LayerDesign{iL}.dkernelmap{i}{j} = convn(flipall(CNN_net.LayerDesign{iL - 1}.a{i}), CNN_net.LayerDesign{iL}.d{j}, 'valid') / size(CNN_net.LayerDesign{iL}.d{j}, 3);
                end
                CNN_net.LayerDesign{iL}.db{j} = sum(CNN_net.LayerDesign{iL}.d{j}(:)) / size(CNN_net.LayerDesign{iL}.d{j}, 3);
            end
        elseif strcmp(CNN_net.LayerDesign{iL}.LayerType, 'F')
            CNN_net.LayerDesign{Num_Layer}.dfc_W= CNN_net.LayerDesign{Num_Layer}.od  *  CNN_net.LayerDesign{Num_Layer}.fv'/ size(CNN_net.LayerDesign{Num_Layer}.od , 2);
            CNN_net.LayerDesign{Num_Layer}.dfc_b=mean(CNN_net.LayerDesign{Num_Layer}.od, 2);  
        end
    end
