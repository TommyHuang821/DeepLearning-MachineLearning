function [Center SIGMA LabeltoCluster J]=kmeans_MD_sheng(data,cluster_n,option)
% k-means based on Mahalanobis distance 
% data: must be n x dim
...
while th>=mincost
    ...
    dis_DatatoCenter=[];
    for c=1:cluster_n
        temp_invsigma=pinv(SIGMA{c});
        temp_center=Center(c,:);

        temp = bsxfun(@minus, data, temp_center);
        temp=diag(temp*temp_invsigma*temp');
